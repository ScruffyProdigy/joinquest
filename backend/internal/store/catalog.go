...
func applyManifestTx(ctx context.Context, tx *sql.Tx, game *Game, manifest *gameclient.Manifest) ([]KickedWaiter, bool, error) {
	...
	for _, modeDef := range manifest.Modes {
		...
		mode, err := upsertGameModeTx(ctx, tx, game.ID, modeKey, displayName, minPlayers, maxPlayers, modeDef.SeatTemplate, modeDef.PreQueue)
		if err != nil {
			return nil, false, err
		}
...
func listGameModes(ctx context.Context, q sqlQueryRowContext, gameID uuid.UUID) ([]GameMode, error) {
	rows, err := q.QueryContext(ctx, `
		SELECT id, game_id, mode_key, display_name, min_players, max_players, pre_queue, status, created_at, updated_at
		FROM game_modes
		WHERE game_id = $1
...
func scanGameModes(rows *sql.Rows) ([]GameMode, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&mode.MinPlayers, &mode.MaxPlayers, &preQueue, &mode.Status,
...
func upsertGameModeTx(ctx context.Context, tx *sql.Tx, gameID uuid.UUID, modeKey, displayName string, minPlayers, maxPlayers int, seatTemplate, preQueue json.RawMessage) (*GameMode, error) {
	row := tx.QueryRowContext(ctx, `
		INSERT INTO game_modes (game_id, mode_key, display_name, min_players, max_players, seat_template, pre_queue, status)
		...
		ON CONFLICT (game_id, mode_key) DO UPDATE SET
			...
			status = 'active',
			updated_at = NOW()
		RETURNING id, game_id, mode_key, display_name, min_players, max_players, seat_template, pre_queue, status, created_at, updated_at
	`, gameID, modeKey, displayName, minPlayers, maxPlayers, seatTemplate, nullJSONColumn(preQueue))
	return scanGameModeRow(row)
}

// nullJSONColumn keeps an absent declaration NULL rather than storing the
// literal bytes "null", so `pre_queue IS NULL` means what it reads like.
func nullJSONColumn(raw json.RawMessage) any {
...
func scanGameModeRow(row *sql.Row) (*GameMode, error) {
	...
	if err := row.Scan(
		&mode.ID, &mode.GameID, &mode.ModeKey, &mode.DisplayName,
		&mode.MinPlayers, &mode.MaxPlayers, &mode.SeatTemplate, &preQueue, &mode.Status,
		&mode.CreatedAt, &mode.UpdatedAt,
	); err != nil {
		return nil, err
	}
