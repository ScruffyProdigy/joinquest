...
| Roster reads *away* | 30s | `store.DefaultRoomMemberAwayGrace` |
| Closed room retention | 6h | `store.DefaultClosedRoomRetention` |

**Only the away window is about display.** The other three take something away from a
...
the roster honest — so it is free to move without touching seat availability.
...
dimmed and labelled *away* (`Room.members[].away`, derived on read from `user_presence` and
stored nowhere), and keeps their membership, seat and chat throughout. So the 5m is no
