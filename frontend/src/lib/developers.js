...
export const CHECK_FIX_HINTS = {
  ...
  'manifest.status': 'GET /api/v1/status should return game info and launchUrlsOnProvision: true.',
  'manifest.launch_urls_on_provision':
    'GET /api/v1/status must include launchUrlsOnProvision: true.',
  'manifest.game_modes': 'GET /api/v1/game-modes needs valid seatTemplate JSON for each mode.',
  'manifest.sync_freshness':
    'Call syncMyGameManifest (dashboard: Resync game modes) after deploying API changes, then re-run checks.',
  'provision.happy_path': 'POST /api/v1/matches should return launch URLs for each seated player.',
  'provision.idempotent_repush':
  ...
  'provision.missing_auth': 'Reject provision requests with no Authorization header.',
  'provision.banlist': 'Return HTTP 403 with bannedLobbyUserIds when a player is banned.',
  'provision.launch_urls': 'Every seated player needs a launchUrls entry (or a launchUrlTemplate).',
  'provision.launch_url_no_jwt':
