...
export const DEVELOPER_PROMO_TITLE_NO_RESULTS =
  "Don’t see your game? Build it.";
export const DEVELOPER_PROMO_BODY =
  "List it on JoinQuest via the developer portal";
...
export const IDENTITY_GATE_ERROR =
  "Could not start your session. Pick an avatar to try again.";
...
export const NAME_PROMPT_TAGLINE =
  "We have your avatar — we just need a name to go with it.";
...
export const AVATAR_PROMPT_TAGLINE =
  "We have your name — we just need a face to go with it.";
export const AVATAR_PROMPT_SCOPE_HINT =
  "This is your avatar across all of JoinQuest, not just one game.";
export const AVATAR_PROMPT_ERROR =
  "Could not save your avatar. Pick one to try again.";
...
export const LAUNCH_HELD_HINT =
  "Countdown paused while you were away. Launch when you are ready.";
...
export function waitingPageSubline(
  gameName,
  modeName,
  roleLabel,
  selectedOptions,
) {
  const parts = [gameName, modeName, roleLabel]
    .map((part) => part?.trim())
    .filter(Boolean);
...
function formatFormingGapParts(formingGaps) {
  ...
  return formingGaps
    ...
    .map((gap) => {
      ...
      const label =
        gap.displayName?.trim() || gap.queuePath?.trim() || "player";
      return `${count} ${label}`;
    });
}
...
/**
 * The solo second action. A solo player's primary action replays the role and the
 * pre-queue options they just had, so this is the way to reach those choices instead of
 * inheriting them — it goes back to the game, where the picker already lives (JQ-232).
 * Absent, not disabled, for a mode with nothing to choose.
 */
...
/** The way out of the group screen that is not "leave" — the rejoin surface is a destination, not a trap. */
...
export const RESULTS_LIVE_UPDATES_OFF =
  "Live updates are off. Reload to see the latest.";
...
export const REGROUP_ERROR_TABLE_FULL =
  "The table filled up before you got a seat.";
export const REGROUP_ERROR_GENERIC =
  "Could not start another round. Try again.";
