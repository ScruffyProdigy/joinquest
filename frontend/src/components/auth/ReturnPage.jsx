...
export default function ReturnPage() {
  ...
  if (status === "result" && result) {
    ...
    const others = (result.participants ?? []).filter(
      (participant) => participant.user?.id !== viewerId,
    );
          ...
          {complete ? (
            <>
              ...
              <RegroupCard
                ...
                // (JQ-232). Declining first is right — this player is not taking the
                // fast path back to this table, and the others should see that.
