...
function makeRoom(table) {
  return {
    ...
    members: [
      { id: "u1", displayName: "Pat" },
      { id: "u7", displayName: "Sam" },
    ],
    tables: [table],
  };
}
...
function makeTable(overrides = {}) {
  return {
    ...
    formingGaps: [
      { queuePath: "Player", displayName: "Player", assigned: 0, needed: 2 },
    ],
    lookForGroupOptions: [],
    regroupRoster: [],
    ...overrides,
...
describe("GroupPage", () => {
  it("renders the group heading and what the table still needs", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Your Group" }),
    ).toBeInTheDocument();
  ...
  it("never shows the viewer as awaiting, whatever their regroup answer says", () => {
    ...
    const you = screen
      .getByRole("region", { name: /picking a seat/i })
      .querySelector("li");
  ...
  it("names who a seated player is waiting on, without saying king", () => {
    ...
    expect(
      screen.getByText("You'll be taken in automatically"),
    ).toBeInTheDocument();
  ...
  it("leaves the table when the player navigates back", async () => {
    ...
    await waitFor(() =>
      expect(mutations.leaveTable).toHaveBeenCalledWith("table-1"),
    );
  ...
  it("discards the table when the last seated player leaves", async () => {
    ...
    await waitFor(() =>
      expect(mutations.discardTable).toHaveBeenCalledWith("table-1"),
    );
  });
  ...
  it("shows the share link so friends can join", () => {
    ...
    expect(
      screen.getByRole("region", { name: /invite friends/i }),
    ).toBeInTheDocument();
    expect(screen.queryByText(/room code/i)).not.toBeInTheDocument();
  });

  // The rejoin surface is a destination, not a trap: a player who would rather go back
  // to matchmaking must be able to say so here, in words (JQ-232).
  it("offers a named way back to matchmaking", () => {
    render(<GroupPage />);

    expect(
      screen.getByRole("link", { name: "Find something new" }),
  ...
  describe("a mode with pre-queue options", () => {
    ...
    it("opens the picker instead of seating the player straight away", async () => {
      ...
      expect(await screen.findByRole("dialog")).toHaveTextContent(
        "Choose your two helpers",
      );
      expect(mutations.sitAtTable).not.toHaveBeenCalled();
      expect(fetchModeQueueOptions).toHaveBeenCalledWith(
        "game-1",
        "mode-1",
        "u1",
      );
    ...
    it("pre-selects nothing, so the seat cannot be claimed without answering", async () => {
      ...
      expect(
        screen.getByRole("button", { name: "Take this seat" }),
      ).toBeDisabled();
    });

    // The roster comes only from the game, so an unreachable game must say so rather
    // than seating the player with no picks at all.
    it("says so when the roster cannot be reached", async () => {
      const user = userEvent.setup();
      fetchModeQueueOptions.mockRejectedValue(
        new Error("Word Hunt is not answering."),
      );
      currentRoom = makeRoom(tableWithOptions());
      render(<GroupPage />);
