...
describe("RegroupCard", () => {
  ...
  it("sends a player who is already in back to the table instead of asking again", () => {
    render(<RegroupCard result={base} viewerId="a" minPlayers={2} />);
    expect(
      screen.getByRole("button", { name: "Back to the table" }),
    ).toBeEnabled();
    expect(
      screen.queryByRole("button", { name: "Another round" }),
    ).not.toBeInTheDocument();
  });

  it("offers the mode switch only for multi-mode games", () => {
    const { unmount } = render(
      <RegroupCard result={base} viewerId="a" minPlayers={2} />,
    );
    expect(
      screen.getByRole("button", { name: "Back to Word Hunt" }),
    ).toBeInTheDocument();
    unmount();

    const singleMode = {
      ...base,
      game: { ...base.game, modes: [{ id: "1" }] },
    };
    render(<RegroupCard result={singleMode} viewerId="a" minPlayers={2} />);
    expect(
      screen.queryByRole("button", { name: "Back to Word Hunt" }),
    ).not.toBeInTheDocument();
  });
  ...
  it('names the viewer as "You" and always offers a way out', () => {
    ...
    expect(
      screen.getByRole("button", { name: "Find something new" }),
    ).toBeInTheDocument();
  });
  ...
  it("disables the primary action while a regroup request is in flight", () => {
    render(
      <RegroupCard result={allPending} viewerId="a" minPlayers={2} busy />,
    );
    expect(
      screen.getByRole("button", { name: "Another round" }),
    ).toBeDisabled();
  });

  /**
   * The solo rule and the group rule are different by design (JQ-232). A solo player's
   * primary action replays the role and options they just had, so they are the only ones
   * who need a way to reach those choices again — a group was never given them back.
   */
  describe("the solo second action", () => {
    const solo = {
      ...allPending,
      groupPlay: false,
      mode: { hasPreMatchChoice: true },
    };
    ...
    it("leaves it out entirely when the mode has nothing to choose", () => {
      ...
      render(
        <RegroupCard result={nothingToChoose} viewerId="a" minPlayers={2} />,
      );
      expect(
